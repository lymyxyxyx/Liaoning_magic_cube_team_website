...
export async function WeeklyCurrentResults({ meet }: { meet: WeeklyMeetOption }) {
  ...
  return <section className="container section weekly-detail-content">
    ...
    <div className="weekly-event-stack">{eventResults.map(({ config, results }) => <section className="weekly-event-section" id={`weekly-event-${config.eventId}`} key={config.eventId}><div className="section-header"><div><span className="eyebrow">{config.eventId}</span><h2>{getWcaEventName(config.eventId)}</h2></div></div>{results.length ? <div className="result-table-wrap"><table className="result-table"><thead><tr><th>排名</th><th>姓名</th><th>组别</th><th>段位</th><th>等级</th><th>平均</th><th>本周最快</th></tr></thead><tbody>{results.map((result) => <tr key={result.id}><td>{result.rank}</td><td>{result.player.name}{result.isNewPlayer ? <small className="weekly-new-player-badge">（新）</small> : null}</td><td>{result.player.ageGroup || "待补"}</td><td><span className={`level-pill level-${result.level}`}>{result.level || "-"}</span></td><td className="grade-cell">{result.grade || "-"}</td><td className="score-strong">{formatResult(result.average)}</td><td>{formatResult(result.best)}</td></tr>)}</tbody></table></div> : <p className="empty-state">本项目暂无已录入成绩。</p>}</section>)}</div>
  </section>;
}
