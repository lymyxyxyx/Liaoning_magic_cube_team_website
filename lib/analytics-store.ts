...
export async function recordPageView(input: { path: string; referrer?: string; userAgent?: string }) {
  ...
  await getPostgresPool().query(
    `
      INSERT INTO page_views (path, referrer, user_agent, device_type)
      VALUES ($1, $2, $3, $4)
    `,
    [path, (input.referrer || "").slice(0, 500), userAgent, getDeviceType(userAgent)]
  );
}
...
export async function getAnalyticsSummary(): Promise<AnalyticsSummary> {
  ...
  const [total, today, yesterday, last7Days, topPages, dailyViews] = await Promise.all([
