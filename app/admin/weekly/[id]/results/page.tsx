...
import { listWeeklyMeetEventConfigs, listWeeklyMeetOptions, listWeeklyMeetResultEventIds } from "@/lib/weekly-entry-store";
...
export default async function WeeklyResultsAdminPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const players = library.map((player) => ({ id: player.id, name: player.name, slug: "", wcaId: player.wcaId || "", wcaIdConfirmed: Boolean(player.wcaIdConfirmed), gender: player.gender === "女" ? "女" as const : player.gender === "男" ? "男" as const : "" as const, province: player.province, city: player.city, birthDate: player.birthDate, ageGroup: player.ageGroup || "", ageGroupIsFuzzy: Boolean(player.ageGroupIsFuzzy), weeklyNumber: weeklyNumberByPlayerId.get(player.id) }));
  return <><PageHero label="后台管理 / 周赛成绩" title={meet.title}>只允许关联 active 的 player_id；修改和删除都会写入修订记录。</PageHero><section className="container weekly-admin-toolbar"><Link className="button" href="/admin/weekly?view=management">返回周赛管理</Link><Link className="button primary" href={`/admin/weekly/${encodeURIComponent(id)}/results/import`}>导入成绩</Link><a className="button" href={`/api/admin/weekly-meets/${encodeURIComponent(id)}/results-export`}>导出 Excel</a></section><WeeklyResultEntryConsole initialMeets={meetOptions} initialPlayers={players} events={WCA_EVENTS} initialEventConfigs={configs} initialResultEventIds={resultEventIds} mode="admin" initialAdminUnlocked /></>;
}
