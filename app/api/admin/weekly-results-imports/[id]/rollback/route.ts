...
import { rollbackWeeklyResultsImportBatch } from "@/lib/weekly-results-import-store";
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json({ batch: await rollbackWeeklyResultsImportBatch({ id: (await params).id, meetId, actor: "weekly-admin" }) });
  } catch (error) {
    return NextResponse.json({ message: error instanceof Error ? error.message : "回滚成绩导入失败" }, { status: 400 });
  }
}
