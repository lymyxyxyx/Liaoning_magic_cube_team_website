...
import { buildWeeklyProvincialRankingView } from "@/lib/weekly-provincial-ranking-view";
...
export default async function WeeklyProvincialRankingsPage({ searchParams }: { searchParams: Promise<{ event?: string; ageGroup?: string }> }) {
  ...
  const visibleRankings = buildWeeklyProvincialRankingView(rankings, selectedAgeGroup);
  ...
  return <>
    ...
    <section className="container section weekly-provincial-ranking-page">
      ...
      <div className="weekly-provincial-ranking-note"><strong>{selectedName}平均成绩榜</strong><span>每位选手仅保留纳入范围内的历史最好平均；组别按创造该成绩时记录，筛选后按组内重新排名。</span></div>
      ...
      <div className="result-table-wrap"><table className="result-table weekly-provincial-ranking-table"><thead><tr><th>{selectedAgeGroup ? `${selectedAgeGroup}组内排名` : "辽宁省排名"}</th><th>选手编号</th><th>姓名</th><th>组别</th><th>性别</th><th>WCA ID</th><th>最好平均</th><th>达成周赛</th></tr></thead><tbody>
